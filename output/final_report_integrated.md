...
- **3.3 關鍵決策矩陣：比較薪資與報名人數的年度增長率 (Decision Matrix: Comparing Annual Growth Rates of Salary and Enrollment)**
...
_圖 3.3.1：行業薪資年增率 vs. 科系報名人數年增率 (110-113 年)_
...
在 3.3 節的視覺化分析中，圖中的迴歸趨勢線呈現非常平緩的負斜率，初步顯示「薪資年增率」與「報名人數年增率」之間可能並無顯著的正向線性關係。為了用更嚴謹的統計方法來檢驗這個視覺觀察，本節將建立一個以「增長率」為核心變數的進階迴歸模型。
...
2.  **自變數的影響**：
    ...
    - 更值得注意的是，其係數估計值為 **-0.146 (負數)**，這進一步證實了 3.3 節中視覺判讀可能具有誤導性，以及迴歸趨勢線實質上是一個非常平緩的負斜率，而非顯著的正向關聯。
...
本節的嚴謹統計檢定證實，3.3 節散佈圖中觀察到的趨勢線雖然存在，但其斜率不具備統計顯著性，且方向為微弱的負相關。這強烈暗示，至少在本研究的資料範圍與模型設定下，「薪資增長」並非影響「報名人數變化」的直接、線性驅動因素。學生的選擇行為，其背後的驅動因子遠比單純的薪資變化更為複雜，這也印證了我們在第一個迴歸模型中得到的初步結論。
