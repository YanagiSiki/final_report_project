...
![各行業平均總薪資盒鬚圖](figures/3_2_salary_boxplot.svg)
_圖 3.2.2：各行業平均總薪資盒鬚圖 (109-113 年)。盒身代表薪資的中位數與四分位距，顯示了該行業在五年內的薪資波動範圍。_

從圖 3.2.1 可見，行業間的薪資水平存在顯著且穩固的差異。圖 3.2.2 則進一步顯示，高薪行業（如金融保險、資通訊）不僅中位數高，其分佈區間也相對較寬，暗示了內部薪資成長的動能較強；而低薪行業（如餐飲、休閒服務）則呈現分佈窄、位置低的特徵，顯示其薪資結構較為僵固。
...
![行業薪資與群類人數變動率散佈圖](figures/3_3_quadrant_chart.svg)
