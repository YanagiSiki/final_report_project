...
- **`output/`**：產生的圖表與文字結果，以及整合與學習用文件。
  - `final_report_integrated.md`：**各章節內容整合檔，非最終報告**，作為後續自動化產生 Word 報告（如 Gemini 工具）之依據。內容需保持章節結構完整，便於轉換。
  - `statistical_methods.md`：**統整本報告分析所用統計方法的學習文件**，供組員查閱與理解。內容需隨分析進度**定期同步更新**，確保涵蓋所有已用統計方法並與主體分析一致。
  - `figures/`：所有統計圖表輸出。
...
| TCTE Group (統測群類)  | Industry (行業別)          | Attribute (特徵)        |
| :--------------------- | :------------------------- | :---------------------- |
| **餐旅群**             | **住宿及餐飲業**           | 薪資水準較低 / 流動率高 |
| **電機與電子群資電類** | **出版影音及資通訊業**     | 薪資水準較高 / 科技導向 |
| **商業與管理群**       | **金融及保險業**           | 薪資水準較高 / 服務業   |
| **機械群**             | **製造業**                 | 傳統產業                |
| **動力機械群**         | **製造業**                 | 傳統產業                |
| **化工群**             | **製造業**                 | 傳統產業                |
| **土木與建築群**       | **營建工程業**             | 薪資較高 / 勞動負荷較高 |
| **衛生與護理類**       | **醫療保健業**             | 薪資穩定 / 人力需求高   |
| **藝術群影視類**       | **藝術﹑娛樂及休閒服務業** | 興趣導向                |

**目前狀態**：分析已完成，報告撰寫中。主要發現為「絕對薪資水準」會影響市場佔有率，而「薪資成長率」對市佔率成長無顯著影響。
**發現**：產業間存在顯著差異，可分為三層級（第一層級：科技/金融，第二層級：醫療/營建，第三層級：餐旅）。
...
- **Model B (Dynamic)**：$ \Delta Y*{share} = \beta_0 + \beta_1 \Delta X*{salary} $
  - **結果**：**顯著**。高絕對薪資對應較高的市場佔有率。
  - **結果**：**不顯著**。短期薪資成長對市場佔有率成長無顯著影響。
- **結論**：學生反應於「存量」(Stock, Current High Salary)，而非「流量」(Flow, Growth Rate)。

## 5. 撰寫規範

- **final_report_integrated.md（章節整合檔）**：
  - 著重章節結構完整、內容整合，格式適合自動轉換產生正式報告（如 Word 檔）。
  - 明確註明「非最終報告」，僅作為自動化產出前的整合依據。
  - 內容需與各章節 Markdown 檔案、統計結果保持一致。
- **statistical_methods.md（統計方法學習文件）**：
  - 著重教學導向、易讀性，內容涵蓋本報告分析所用統計方法。
  - 需隨分析內容**定期同步**，確保涵蓋所有已用方法並與主體分析一致。
- **一般規範**：
  - **格式**：Markdown。
